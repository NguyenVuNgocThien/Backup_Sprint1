...
const SelectTable = (props) => {
    const { title, headers, fields, selectedRow, setSelectedRow } = props;

    const [data, setData] = useState([])
    const [rowCount, setRowCount] = useState(0)

    const defaultSortBy = fields[0].charAt(0).toUpperCase() + fields[0].slice(1)
    ...
    useEffect(() => {
        if (title === "User") {
            getUsers(pagination.currentPage, pagination.rowsPerPage, sortBy, searchString, sortType).then((response) => {
            ...
            getAvailableAssets(pagination.currentPage, pagination.rowsPerPage, sortBy, searchString, sortType).then((response) => {
                setRowCount(response.data.total)
    ...
    return (
        <div className="card position-absolute select-user-dropdown py-3 px-4" style={{"width": "750px"}}>
            <div className="row">
            ...
            <table className="table table-borderless">
                <thead>
                    <tr>
                        ...
                        {headers.map((header, index) =>
                            <th key={index} className="p-0 cursor-pointer" onClick={() => onSort(fields[index])}>
                ...
                <tbody>
                    {data.map((row) =>
                        <tr key={row[fields[0]]} className="radio-gender-group" onClick={() => handleOnClick(row[fields[0]])}>
                            <td className="border-0 gender__container">
                                <input
                                    type="radio"
                                    value={row[fields[0]]}
                                    name="radio"
                                    checked={row[fields[0]] === selectedRow[fields[0]]}
                                    onChange={e => { }}
                                />
